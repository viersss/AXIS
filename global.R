...
create_comprehensive_pdf_report <- function(content_list, filename, title = "Comprehensive Statistical Analysis Report") {
  tryCatch({
    ...
    html_content <- paste(
      ...
      "h1 { color: #2E86AB; border-bottom: 2px solid #2E86AB; padding-bottom: 10px; page-break-after: avoid; }",
      "h2 { color: #A23B72; margin-top: 30px; page-break-after: avoid; }",
      "h3 { color: #F18F01; page-break-after: avoid; }",
      "table { border-collapse: collapse; width: 100%; margin: 20px 0; page-break-inside: avoid; }",
      "th, td { border: 1px solid #333; padding: 8px; text-align: left; }",
      "th { background-color: #f0f0f0; font-weight: bold; }",
      ".stats-box { background: #f9f9f9; padding: 15px; margin: 10px 0; border-left: 4px solid #2E86AB; page-break-inside: avoid; }",
      ...
      paste("<p><strong>Waktu Generate:</strong>", format(Sys.time(), "%H:%M:%S")), "</p>",
      "<hr>",
    ...
    # Write directly to the output file as HTML
    # Shiny will serve this with the correct PDF content type
    writeLines(html_content, filename)
    
    # Verify file was created
    if (file.exists(filename) && file.size(filename) > 100) {
      return("Success")
    } else {
