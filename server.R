...
# Server Logic (server.R) - Complete Functional Version with Fixed Notifications
  ...
  output$download_data_report <- downloadHandler(
    ...
    content = function(file) {
      if (!is.null(values$processed_data)) {
        ...
        data_quality <- assess_data_quality(values$processed_data)
        desc_stats <- create_descriptive_stats_table(values$processed_data)
        
        # Create visualizations
        temp_plots <- list()
        
        # Create correlation matrix if possible
        cor_plot_file <- NULL
        if (length(data_quality$numeric_vars) >= 2) {
          cor_plot_file <- create_correlation_matrix(values$processed_data)
        }
        
        # Generate missing data visualization
        missing_plot <- NULL
        if (data_quality$missing_total > 0) {
          temp_missing_file <- tempfile(fileext = ".png")
          png(temp_missing_file, width = 800, height = 600, res = 150)
          VIM::aggr(values$processed_data, col = c(statistical_colors$primary, statistical_colors$warning),
                   numbers = TRUE, sortVars = TRUE)
          dev.off()
          missing_plot <- temp_missing_file
        }
        
        # Build comprehensive content
        content_list <- c(
          generate_executive_summary(values$processed_data, "Manajemen Data"),
          generate_methodology_section("Data Management"),
          
          "# Analisis Kualitas Data Komprehensif",
          "",
          "## Informasi Dataset",
          "",
          paste("- **Total Observasi:** ", format(data_quality$n_rows, big.mark = ","), " unit analisis"),
          paste("- **Total Variabel:** ", data_quality$n_cols, " variabel"),
          paste("- **Variabel Numerik:** ", length(data_quality$numeric_vars), " variabel"),
          paste("- **Variabel Kategorikal:** ", length(data_quality$character_vars) + length(data_quality$factor_vars), " variabel"),
          paste("- **Missing Data Total:** ", format(data_quality$missing_total, big.mark = ","), " (", data_quality$missing_percent, "%)", sep = ""),
          "",
          "### Daftar Variabel Berdasarkan Tipe",
          "",
          "**Variabel Numerik:**",
          if (length(data_quality$numeric_vars) > 0) {
            paste("- ", data_quality$numeric_vars, collapse = "\n")
          } else {
            "- Tidak ada variabel numerik"
          },
          "",
          "**Variabel Kategorikal:**",
          if (length(c(data_quality$character_vars, data_quality$factor_vars)) > 0) {
            paste("- ", c(data_quality$character_vars, data_quality$factor_vars), collapse = "\n")
          } else {
          ...
          if (data_quality$missing_total > 0) {
            c(
              "Dataset mengandung missing data yang perlu diperhatikan:",
              "",
              "### Distribusi Missing Data per Variabel",
              "",
              paste("| Variabel | Missing Count | Missing % |",
                    "|----------|---------------|-----------|",
                    paste("|", names(data_quality$missing_by_var), "|", 
                          data_quality$missing_by_var, "|", 
                          data_quality$missing_percent_by_var, "%|", 
                          collapse = "\n"),
                    sep = "\n"),
              "",
              "### Interpretasi Missing Data",
              "",
              if (data_quality$missing_percent < 5) {
                "- Missing data dalam kategori **rendah** (< 5%)"
              } else if (data_quality$missing_percent < 10) {
                "- Missing data dalam kategori **sedang** (5-10%)"
              } else {
                "- Missing data dalam kategori **tinggi** (> 10%)"
              },
              "",
              "**Rekomendasi:**",
              if (data_quality$missing_percent < 5) {
                "- Listwise deletion dapat digunakan tanpa bias signifikan"
              } else if (data_quality$missing_percent < 10) {
                "- Pertimbangkan imputation atau analisis sensitivitas"
              } else {
                "- Analisis pola missing data diperlukan sebelum treatment"
              }
            )
          } else {
            c(
              "Dataset **tidak mengandung missing data**.",
              "",
              "**Interpretasi:** Ini merupakan kondisi ideal untuk analisis statistik.",
              "Tidak diperlukan treatment khusus untuk missing data."
            )
          },
          "",
          if (!is.null(missing_plot)) {
            c(
              "```{r missing-plot, fig.cap='Pola Missing Data dalam Dataset'}",
              paste("knitr::include_graphics('", missing_plot, "')", sep = ""),
              "```",
              ""
            )
          } else {
          ...
          "## Deteksi Outliers",
          "",
          if (length(data_quality$outliers) > 0) {
            c(
              "Analisis outliers menggunakan metode IQR (Interquartile Range):",
              "",
              "### Jumlah Outliers per Variabel",
              "",
              paste("| Variabel | Jumlah Outliers | Persentase |",
                    "|----------|-----------------|------------|",
                    paste("|", names(data_quality$outliers), "|", 
                          unlist(data_quality$outliers), "|", 
                          round((unlist(data_quality$outliers) / data_quality$n_rows) * 100, 2), "%|", 
                          collapse = "\n"),
                    sep = "\n"),
              "",
              "**Interpretasi Outliers:**",
              "",
              paste("- Total outliers terdeteksi:", sum(unlist(data_quality$outliers))),
              paste("- Persentase outliers:", round((sum(unlist(data_quality$outliers)) / (data_quality$n_rows * length(data_quality$outliers))) * 100, 2), "%"),
              "",
              "**Rekomendasi Treatment:**",
              if (sum(unlist(data_quality$outliers)) / data_quality$n_rows < 0.05) {
                "- Outliers dalam batas wajar (< 5%)"
              } else {
                "- Perlu investigasi lebih lanjut untuk outliers"
              }
            )
          } else {
            "Tidak ada variabel numerik untuk analisis outliers."
          },
          "",
          ...
          if (!is.null(desc_stats)) {
            c(
              "## Ringkasan Statistik Semua Variabel Numerik",
              "",
              "Tabel berikut menyajikan statistik deskriptif lengkap untuk semua variabel numerik:",
              "",
              "```{r desc-stats-table}",
              "library(knitr)",
              "desc_stats <- data.frame(",
              paste("  Variable = c(", paste("'", unique(desc_stats$Variable), "'", collapse = ", "), "),", sep = ""),
              paste("  N = c(", paste(desc_stats[desc_stats$Statistic == "N", "Value"], collapse = ", "), "),"),
              paste("  Mean = c(", paste(desc_stats[desc_stats$Statistic == "Mean", "Value"], collapse = ", "), "),"),
              paste("  SD = c(", paste(desc_stats[desc_stats$Statistic == "SD", "Value"], collapse = ", "), "),"),
              paste("  Min = c(", paste(desc_stats[desc_stats$Statistic == "Min", "Value"], collapse = ", "), "),"),
              paste("  Median = c(", paste(desc_stats[desc_stats$Statistic == "Median", "Value"], collapse = ", "), "),"),
              paste("  Max = c(", paste(desc_stats[desc_stats$Statistic == "Max", "Value"], collapse = ", "), "),"),
              paste("  Skewness = c(", paste(desc_stats[desc_stats$Statistic == "Skewness", "Value"], collapse = ", "), "),"),
              paste("  Kurtosis = c(", paste(desc_stats[desc_stats$Statistic == "Kurtosis", "Value"], collapse = ", "), ")"),
              ")",
              "kable(desc_stats, caption = 'Statistik Deskriptif Lengkap')",
              "```",
              ""
            )
          } else {
            "Tidak ada variabel numerik untuk analisis deskriptif."
          },
          
          "### Interpretasi Distribusi Data",
          "",
          if (!is.null(desc_stats) && length(data_quality$numeric_vars) > 0) {
            skewness_values <- desc_stats[desc_stats$Statistic == "Skewness", "Value"]
            kurtosis_values <- desc_stats[desc_stats$Statistic == "Kurtosis", "Value"]
            
            c(
              "**Analisis Skewness (Kemiringan):**",
              "",
              paste("- Variabel dengan distribusi normal (|skewness| < 0.5):", 
                    sum(abs(skewness_values) < 0.5, na.rm = TRUE), "variabel"),
              paste("- Variabel dengan skewness moderat (0.5 ≤ |skewness| < 1):", 
                    sum(abs(skewness_values) >= 0.5 & abs(skewness_values) < 1, na.rm = TRUE), "variabel"),
              paste("- Variabel dengan skewness tinggi (|skewness| ≥ 1):", 
                    sum(abs(skewness_values) >= 1, na.rm = TRUE), "variabel"),
              "",
              "**Analisis Kurtosis (Keruncingan):**",
              "",
              paste("- Variabel dengan kurtosis normal (kurtosis ≈ 3):", 
                    sum(abs(kurtosis_values - 3) < 0.5, na.rm = TRUE), "variabel"),
              paste("- Variabel dengan distribusi leptokurtic (kurtosis > 3):", 
                    sum(kurtosis_values > 3, na.rm = TRUE), "variabel"),
              paste("- Variabel dengan distribusi platykurtic (kurtosis < 3):", 
                    sum(kurtosis_values < 3, na.rm = TRUE), "variabel")
            )
          } else {
            "Tidak tersedia data untuk interpretasi distribusi."
          },
          "",
          
          if (!is.null(cor_plot_file)) {
            c(
              "# Analisis Korelasi Antar Variabel",
              "",
              "## Matriks Korelasi",
              "",
              "Analisis korelasi menunjukkan hubungan linear antar variabel numerik:",
              "",
              "```{r correlation-matrix, fig.cap='Matriks Korelasi Antar Variabel Numerik'}",
              paste("knitr::include_graphics('", cor_plot_file, "')", sep = ""),
              "```",
              "",
              ...
              "- Multikolinearitas perlu diperhatikan jika |r| > 0.8",
              "- Variabel dengan korelasi tinggi mungkin redundan",
              "- Korelasi dapat mengindikasikan hubungan kausal yang perlu diuji"
            )
          } else {
            ""
          },
          ...
          "# Rekomendasi Analisis Lanjutan",
          "",
          "Berdasarkan karakteristik data yang ditemukan, berikut rekomendasi analisis selanjutnya:",
          "",
          "## 1. Preprocessing Data",
          "",
          if (data_quality$missing_total > 0) {
            "- **Treatment Missing Data:** Pertimbangkan imputation atau deletion strategy"
          } else {
            "- **Missing Data:** Tidak diperlukan treatment khusus"
          },
          
          if (sum(unlist(data_quality$outliers)) > 0) {
            "- **Treatment Outliers:** Investigasi dan pertimbangkan windsorizing atau removal"
          } else {
            "- **Outliers:** Data relatif bersih dari outliers ekstrem"
          },
          "",
          "## 2. Uji Normalitas",
          "",
          "- Lakukan uji Shapiro-Wilk atau Kolmogorov-Smirnov",
          "- Pertimbangkan transformasi data jika diperlukan",
          "- Gunakan uji non-parametrik jika asumsi normalitas tidak terpenuhi",
          "",
          "## 3. Analisis Inferensial",
          "",
          "- **Uji Hipotesis:** Sesuaikan dengan jenis data dan distribusi",
          "- **Analisis Varians:** ANOVA untuk perbandingan multiple groups",
          "- **Regresi:** Untuk analisis hubungan prediktif",
          "",
          "## 4. Visualisasi Lanjutan",
          "",
          "- Histogram dan Q-Q plots untuk setiap variabel",
          "- Scatterplot matrix untuk eksplorasi hubungan",
          "- Boxplots untuk identifikasi outliers dan distribusi",
          "",
          "---",
          "",
          ...
          "",
          "Untuk analisis lebih lanjut, silakan gunakan fitur-fitur analisis yang tersedia di AXIS Dashboard."
        )
        ...
        if (!is.null(cor_plot_file) && file.exists(cor_plot_file)) {
          unlink(cor_plot_file)
        }
        if (!is.null(missing_plot) && file.exists(missing_plot)) {
          unlink(missing_plot)
        }
