...
calculate_morans_i <- function(x, weights_matrix) {
  ...
  n <- length(x)
  if (n < 2) {
    stop("Insufficient data for Moran's I calculation")
  }
  ...
  sum_sq_dev <- sum(x_dev^2, na.rm = TRUE)
  
  # Calculate weighted sum of products
  weighted_sum <- 0
  total_weight <- 0
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      if (i != j && !is.na(weights_matrix[i, j])) {
        weighted_sum <- weighted_sum + weights_matrix[i, j] * x_dev[i] * x_dev[j]
  ...
  # Calculate Moran's I
  if (total_weight > 0 && sum_sq_dev > 0) {
    morans_i <- (n * weighted_sum) / (2 * total_weight * sum_sq_dev)
  } else {
    morans_i <- 0
  }
