...
server <- function(input, output, session) {
  ...
  output$download_data_report <- downloadHandler(
    filename = function() {
      paste("laporan-data-", Sys.Date(), ".pdf", sep = "")
    },
    content = function(file) {
      if (!is.null(values$processed_data)) {
        content <- paste(
          "# Laporan Data AXIS Dashboard",
          "",
          paste("**Tanggal:**", Sys.Date()),
          "",
          "## Ringkasan Data",
          paste("- Jumlah observasi:", nrow(values$processed_data)),
          paste("- Jumlah variabel:", ncol(values$processed_data)),
          paste("- Missing values:", sum(is.na(values$processed_data))),
          "",
          "## Struktur Data",
          "```",
          capture.output(str(values$processed_data)),
          "```",
          "",
          "## Statistik Deskriptif",
          "```",
          capture.output(summary(values$processed_data)),
          "```",
          sep = "\n"
        )
        
        create_pdf_report(content, file, "Laporan Data AXIS Dashboard")
      }
    }
  )
  ...
  output$download_exploration_report <- downloadHandler(
    filename = function() {
      paste("laporan-eksplorasi-", Sys.Date(), ".pdf", sep = "")
    },
    content = function(file) {
      if (!is.null(values$processed_data) && !is.null(input$explore_variable)) {
        var_data <- values$processed_data[[input$explore_variable]]
        ...
        content <- paste(
          ...
          "",
          paste("**Tanggal:**", Sys.Date()),
          paste("**Variabel:**", input$explore_variable),
          "",
          "## Statistik Deskriptif",
          "",
          if (is.numeric(var_data)) {
            paste(
              ...
              paste("- **Skewness:**", round(moments::skewness(var_data, na.rm = TRUE), 4)),
              paste("- **Kurtosis:**", round(moments::kurtosis(var_data, na.rm = TRUE), 4)),
              sep = "\n"
            )
          } else {
            paste("**Tabel Frekuensi:**", paste(capture.output(table(var_data)), collapse = "\n"))
          },
          "",
          "## Interpretasi",
          "",
          "Analisis statistik deskriptif menunjukkan karakteristik distribusi data yang penting untuk analisis lanjutan.",
          sep = "\n"
        )
        
        create_pdf_report(content, file, "Laporan Eksplorasi Data")
      }
    }
  )
  ...
  output$download_regression_report <- downloadHandler(
    filename = function() {
      paste("laporan-regresi-", Sys.Date(), ".pdf", sep = "")
    },
    content = function(file) {
      content <- paste(
        ...
        "## Model Regresi",
        "",
        "Analisis hubungan antara variabel dependen dengan beberapa variabel independen.",
        "",
        "## Diagnostik Model",
        "",
        "Evaluasi asumsi regresi dan kualitas model yang dibangun.",
        "",
        "## Interpretasi Koefisien",
        "",
        "Penjelasan makna dan signifikansi setiap koefisien dalam model.",
        sep = "\n"
      )
      
      create_pdf_report(content, file, "Laporan Regresi Linear Berganda")
    }
  )
