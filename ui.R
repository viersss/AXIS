...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem(" Laporan Komprehensif", tabName = "laporan_komprehensif", icon = icon("file-word", lib = "font-awesome"))
    )
  ),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "laporan_komprehensif",
              fluidRow(
                box(
                  title = tagList(icon("download"), "Unduh Laporan Analisis Komprehensif"),
                  status = "primary", 
                  solidHeader = TRUE, 
                  width = 12,
                  p("Klik tombol di bawah ini untuk mengunduh laporan analisis lengkap dalam format Microsoft Word (.docx). Laporan ini secara otomatis merangkum semua analisis yang telah Anda lakukan di dasbor, mulai dari eksplorasi data hingga regresi linear berganda.", style = "font-size: 16px;"),
                  p("Setiap bagian disertai dengan output statistik, visualisasi yang relevan, dan interpretasi mendalam dari sudut pandang seorang ahli statistik untuk mendukung pengambilan keputusan berbasis bukti.", style = "font-size: 16px;"),
                  hr(),
                  ...
                  div(style = "text-align: center;",
                      downloadButton("download_word_report", 
                                     ...
                                     style = "font-weight: bold; padding: 15px 30px; font-size: 18px;")
                  ),
                  
                  br(),
                  
                  tags$div(class = "interpretation-box",
                           tags$h5("Harap Diperhatikan"),
                           p(icon("info-circle"), " Proses pembuatan laporan mungkin memerlukan waktu beberapa saat (tergantung pada jumlah analisis yang dijalankan), karena sistem sedang menggabungkan semua hasil dan visualisasi ke dalam satu dokumen yang kohesif. Mohon bersabar.")
                  )
                )
              )
      )
